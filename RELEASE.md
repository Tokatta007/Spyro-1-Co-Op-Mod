...
# v0.3: per-player color

Give each dragon his own color, from a page in the game's own pause menu.
...
- **A live color preview** above the box for each player, framed in the game's
  own shimmering gold. It shows the blended result rather than the raw value,
  so turning the tint down visibly walks back toward Spyro's purple.
- **The d-pad steps by one, and L2 and R2 by sixteen.** SQUARE restores the
  column your cursor is in.
- Colors **hold through portals, level entrances, dialogue, and exits**, so the
  dragons stay themselves through a whole session.
- Defaults are **Spyro exactly as he ships**, so a fresh start looks like
  retail, and any color is one you chose.

This tints the dragon rather than repainting his textures, the same thing
*Spyro 2*'s color cheat codes did, and with the same character: a yellow Spyro
is a yellow-tinted Spyro, not a repainted one.
...
| file | |
| --- | --- |
| `spyro1-coop-v0.3.xdelta` | the patch, applied to your own disc image |
...
- Patch it with [Delta Patcher](https://github.com/marco-calautti/DeltaPatcher),
  or online at [romhacking.net](https://www.romhacking.net/patch/), which needs
  nothing installed
...
- **Camera glitch on some hits.** Being hit, by a charging ram especially, can
...
- **Player 2 talking to the balloonist** shows player 1's color for the
  duration of the conversation. Cosmetic, and the fix does not fit in the
  space left.
- **Four-player split is not achievable on PS1 hardware.** It remains the goal,
  but the scene is drawn once per viewport, and four measured at half the
